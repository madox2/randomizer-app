...
export class Matches extends Component {
  ...
  getNewStates(count) {
    return new Array(count).fill(false).map((v, idx) => {
      const pulled = false
  ...
  showMatch(idx, position) {
    Animated.timing(position, {
      ...
      this.setState({
        matches: this.state.matches.map((s, i) =>
          i !== idx
            ? s
            : {
                ...s,
  ...
  updateLayoutProperties(state) {
    ...
    const pullHeight = Math.min(
      availableHeight - this.matchHeight,
      this.matchHeight / 4,
    )
    this.lowerPosition = -Math.max(
      (availableHeight - this.matchHeight - pullHeight) / 2,
    )
  ...
  render() {
    ...
    return (
      <SectionTemplate
        ...
        title="Matches"
        style={s.container}>
        <ScrollView contentContainerStyle={s.scrollView} horizontal={true}>
          {matches.map((match, i) => (
            <Animated.View
              key={`${throwNumber}-${i}`}
...
const makeStyles = ResponsiveStyleSheet.create(
  ({contentWidth, controlsHeight, matchHeight, contentPadding, matchCount}) => {
